...
function displayBoxOne(){
  let currentTime = weatherForecastObj.currentTime
...
function displayNextHours(){
  let hours = weatherForecastObj.nextHours.splice(0,8);
...
function displayHour(hour_data){
  ...
  let hour = timeConverter(hour_data["time"]);
...
function displayUpcomingDay(day_data){
  ...
  let dayName = timeConverter(day_data["time"]);
